...
export async function POST(req: Request) {
  ...
  // 【重要】ドメイン浸透までは Vercel の URL を優先的に使用する
  const baseUrl = "https://ai-review-site-nine.vercel.app"; 

  if (!stripeKey || !resendKey || !webhookSecret || !sig) {
    console.error("❌ Configuration missing in Vercel Environment Variables");
          ...
          html: `
            <div style="font-family: sans-serif; max-width: 600px; margin: auto; padding: 30px; border: 1px solid #000; background: #fff;">
              ...
              <p style="font-size: 12px; color: #666; margin-top: 30px;">
                Note: This link is unique to your transaction. Unauthorized distribution will lead to immediate account termination.
              </p>
            </div>
          `
        });
